...
# pip install --upgrade edge-tts pygame SpeechRecognition requests langdetect PyAudio
...
def calc_score(target, said):
    ...
    # 단어의 중복 횟수까지 반영하여 같은 단어를 반복해 점수가 부풀지 않게 합니다.
...
# MultiLanguageAI.json의 tts_code 값을 사용해 Edge TTS 음성을 선택합니다.
# ------------------------------------------------------------------------
EDGE_TTS_VOICE_MAP = {
...
_tts_cache_locks = defaultdict(threading.Lock)
...
def synthesize_tts_to_file(text, language_code, cache_path):
    ...
    성공 시 True를 반환합니다.
    """
    with _tts_cache_locks[str(cache_path)]:
...
class StatsWindow(tk.Toplevel):
    ...
    def refresh_list(self):
        ...
        for history_index in range(len(self.history) - 1, -1, -1):
            e = self.history[history_index]
            self.tree.insert(
                "",
                "end",
                iid=f"history-{history_index}",
                values=(e["date"], f"{e['score']}%")
            )
    ...
    def delete_selected(self):
        ...
        for item in selected:
            try:
                indices.append(int(str(item).split("-", 1)[1]))
            except (IndexError, ValueError):
                logger.error(f"학습 기록 식별자 오류: {item}")
...
class MultiLangAIApp(tk.Tk):
    ...
    def show_translation(self):
        ...
        request_idx = self.curr_idx
        request_text = self.sentences[request_idx]
        source_lang = self.current_lang_code
        target_lang = self.trans_target_code
        # 작업 중 언어·문장을 바꿔도 요청이 처음 상태를 유지하도록 스냅샷을 전달합니다.
        EXECUTOR.submit(
    ...
    def _translate_thread(self, request_idx, target_text, source_lang, target_lang):
        try:
    ...
    def play_tts(self):
        ...
        request_idx = self.curr_idx
        text = self.sentences[request_idx]
        language_code = self.current_lang_info["tts_code"]
        language_name = self.current_lang_info["name"]
        cache_path = CACHE_DIR / f"{language_code}_{hashlib.md5(text.encode('utf-8')).hexdigest()}.mp3"

        # [최적화 ⑦] 같은 문장에 대해 mp3 캐시 파일이 아직 없는 상태에서 버튼을
        # 연타하면, 두 스레드가 동시에 같은 파일에 TTS 저장을 시도해 충돌할 수
        ...
        def _play_audio():
            try:
                ...
                self.after(0, lambda: messagebox.showwarning(
                    "TTS 미지원", 
                    f"현재 언어({language_name})는 음성 듣기를 지원하지 않습니다."
                ))
            except Exception as e:
    ...
    def start_stt(self):
        ...
        EXECUTOR.submit(self._listen_thread, request_idx, target_text, stt_code)

    # _listen_thread 메서드는 별도의 스레드에서 음성 인식을 수행하고, 인식 결과를 UI에 업데이트합니다.
    def _listen_thread(self, request_idx, target_text, stt_code):
        try:
            r = sr.Recognizer()
            ...
            with sr.Microphone() as source:
                ...
                self.after(0, lambda: self.btn_mic.config(
                    text="● 지금 말씀하세요",
                    fg=C["danger"]
                ))
                self.after(0, lambda: self.lbl_result.config(
                ...
                self.after(0, lambda: self.btn_mic.config(text="분석 중..."))

                said = r.recognize_google(audio, language=stt_code)
                score = calc_score(target_text, said)
