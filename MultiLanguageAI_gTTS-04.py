...
required_packages = {
    ...
    "requests": False,
    "langdetect": True,
    "edge_tts": False   # gTTS가 지원하지 않는 언어(uz, lo 등)의 TTS 보완용
}

# [최적화 ①] speech_recognition, gTTS, pygame, requests는 원래 함수 호출 시점마다
# 함수 내부에서 반복적으로 import 되었습니다. sys.modules에 캐시되어 있어 두 번째
# 호출부터는 느리지 않지만, 호출될 때마다 불필요한 속성 조회/바이트코드 실행이
# 발생합니다. 여기서 한 번만 import하고 아래에서는 모듈 전역 변수(sr, gTTS, pygame,
...
def check_packages():
    global sr, gTTS, pygame, requests, edge_tts
    try:
    ...
    try:
        ...
        import pygame
        required_packages["gTTS"] = True
        required_packages["pygame"] = True
    except ImportError as e:
        logger.error(f"gTTS 또는 pygame 패키지 없음: {e}")
        messagebox.showerror("패키지 오류", f"gTTS 또는 pygame 패키지가 설치되지 않았습니다: {e}")
    ...
    # edge-tts는 gTTS가 지원하지 않는 일부 언어(우즈베크어 uz, 라오어 lo 등)의
    # 발음 재생을 대체하기 위한 보조 패키지입니다. 없어도 프로그램 전체가
    # 동작하는 데는 문제가 없으므로(해당 언어 발음 재생만 제한됨) 오류창을
    # 띄우지 않고 로그만 남깁니다.
    try:
        import edge_tts
        required_packages["edge_tts"] = True
    except ImportError:
        logger.warning("edge-tts 패키지 없음 (우즈베크어/라오어 등 일부 언어의 발음 재생이 제한됩니다)")
...
# Google Translate GTX API를 통해 번역을 수행하는 함수
# source_lang -> target_lang 으로 자유롭게 번역 가능하도록 일반화
# (기존에는 target_lang이 "ko"로 고정되어 있었음)
    ...
    for attempt in range(1, _TRANSLATE_MAX_RETRIES + 1):
        ...
        try:
            ...
            if r.status_code == 429:
                # 구글 쪽에서 요청 제한이 걸린 경우. 즉시 재시도해도 대부분
                # 다시 429가 나므로, 점점 길어지는 대기시간을 두고 재시도합니다.
                wait_sec = _TRANSLATE_RETRY_BACKOFF_SEC * attempt
                logger.warning(
                    f"구글 번역 429(요청 제한) 발생, {wait_sec:.0f}초 후 재시도 "
                    f"({attempt}/{_TRANSLATE_MAX_RETRIES})"
                )
                if attempt < _TRANSLATE_MAX_RETRIES:
                    time.sleep(wait_sec)
                    continue
                return "(번역 서비스 요청이 많아 일시적으로 제한되었습니다. 잠시(1분 정도) 후 다시 시도해 주세요.)"
            ...
            # 429가 아닌 다른 오류(네트워크 오류, 타임아웃 등)는 기존과 동일하게
            # 재시도 없이 바로 에러 메시지를 반환합니다.
            logger.error(f"번역 오류: {e}")
            return "(번역 중 오류가 발생했습니다. 인터넷 연결을 확인하고 다시 시도해 주세요.)"

    # 이론상 도달하지 않지만 안전망으로 남겨둠
    return "(번역 서비스 요청이 많아 일시적으로 제한되었습니다. 잠시 후 다시 시도해 주세요.)"

# safe_pygame_init 함수는 Pygame의 mixer 모듈을 초기화합니다. 
# gTTS와 pygame 패키지가 설치되어 있지 않으면 에러 메시지를 출력하고 False를 반환합니다.

# Pygame을 안전하게 초기화하는 함수
def safe_pygame_init():
    if not (required_packages["gTTS"] and required_packages["pygame"]):
        logger.error("gTTS 또는 pygame 패키지 없음")
        return False
    try:
...
# ------------------------------------------------------------------------
# gTTS가 지원하지 않는 언어를 위한 Microsoft Edge TTS(edge-tts) 보완
#
# gTTS(Google Translate TTS)는 우즈베크어(uz), 라오어(lo) 등 일부 언어를
# 지원하지 않아 gTTS(text=..., lang="uz")처럼 호출하면 ValueError가 발생합니다.
# 이 딕셔너리에 등록된 gtts 코드는 gTTS를 아예 시도하지 않고 곧바로
# edge-tts(마이크로소프트 엣지 브라우저가 사용하는 온라인 TTS)로 음성을
# 생성합니다. 새로 지원이 필요한 언어가 생기면 여기에만 추가하면 됩니다.
# (edge-tts 지원 음성 전체 목록은 명령행에서 `edge-tts --list-voices`로 확인 가능)
# ------------------------------------------------------------------------
EDGE_TTS_VOICE_MAP = {
    "uz": "uz-UZ-SardorNeural",       # 우즈베크어 (남성). 여성: uz-UZ-MadinaNeural
    "lo": "lo-LA-ChanthavongNeural",  # 라오어 (남성). 여성: lo-LA-KeomanyNeural
}
...
#     문제없이 재생됩니다.
#
# 해결: gtts_code가 "uz"이고 텍스트에 키릴 문자가 섞여 있으면, edge-tts로
...
def _prepare_tts_text(text, gtts_code):
    """edge-tts로 보내기 직전, 필요하다면 언어별 텍스트 전처리를 적용합니다."""
    preprocessor = _TTS_TEXT_PREPROCESSORS.get(gtts_code)
    return preprocessor(text) if preprocessor else text

class TTSUnavailableError(Exception):
    """gTTS도, edge-tts 대체 음성도 사용할 수 없어 발음 재생이 불가능할 때 발생시키는 예외."""
    def __init__(self, gtts_code):
        self.gtts_code = gtts_code
        super().__init__(f"'{gtts_code}' 언어는 재생 가능한 TTS 엔진이 없습니다.")
...
def synthesize_tts_to_file(text, gtts_code, cache_path):
    """
    ...
    - gtts_code가 EDGE_TTS_VOICE_MAP에 등록된 언어(uz, lo 등)라면
      gTTS를 시도하지 않고 바로 edge-tts로 합성합니다.
    - 그 외 언어는 기존과 동일하게 gTTS를 사용하되, gTTS가 뜻밖에
      ValueError(미지원 언어)를 던지는 경우에도 edge-tts 대체 음성이
      있다면 한 번 더 시도합니다.
    - edge-tts가 필요한데 패키지가 설치되어 있지 않으면
      TTSUnavailableError를 발생시킵니다 (호출부에서 안내 메시지 처리).
    ...
    edge_voice = EDGE_TTS_VOICE_MAP.get(gtts_code)

    # gTTS가 원천적으로 지원하지 않는 것으로 이미 알려진 언어는
    # 굳이 gTTS를 호출해 실패시키지 않고 바로 edge-tts를 사용합니다.
    if edge_voice:
        if not required_packages.get("edge_tts"):
            raise TTSUnavailableError(gtts_code)
        # [버그수정] 키릴 문자로 쓰인 우즈베크어 등은 edge-tts 음성이 인식하지
        # 못하므로, 전송 직전에 라틴 문자로 변환합니다(_prepare_tts_text).
        tts_text = _prepare_tts_text(text, gtts_code)
    ...
    try:
        ...
        # gTTS 미지원 언어로 확인된 경우, edge-tts 대체 음성이 있으면 재시도
        if edge_voice and required_packages.get("edge_tts"):
            tts_text = _prepare_tts_text(text, gtts_code)
            _synthesize_with_edge_tts(tts_text, edge_voice, cache_path)
            return True
        raise
...
def _synthesize_and_play(text, gtts_code, cache_path, max_attempts=2):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        synthesize_tts_to_file(text, gtts_code, cache_path)
        try:
...
class MultiLangAIApp(tk.Tk):
    ...
    def play_translated_tts(self, text, lang_code):
        ...
        cache_path = CACHE_DIR / f"{gtts_code}_{hashlib.md5(text.encode('utf-8')).hexdigest()}.mp3"

        # _play_audio 메서드는 별도의 스레드에서 TTS(gTTS 또는 edge-tts)를 사용하여 번역된 문장을 음성으로 재생합니다.
        def _play_audio():
            try:
                _synthesize_and_play(text, gtts_code, cache_path)
            except TTSUnavailableError:
                # gTTS도 edge-tts도 사용할 수 없는 경우(예: edge-tts 미설치)
                logger.warning(f"번역 음성 미지원 언어: {gtts_code} ({lang_info['name']})")
                self.after(0, lambda: self.lbl_result.config(
                    text=f"⚠ 이 언어({lang_info['name']})는 발음 재생을 지원하지 않습니다.",
                    fg=C["warn"]
                ))
            except ValueError:
                # gTTS가 지원하지 않고 edge-tts 대체 음성도 없는 언어 코드인 경우
                logger.warning(f"번역 음성 미지원 언어: {gtts_code} ({lang_info['name']})")
                self.after(0, lambda: self.lbl_result.config(
                    text=f"⚠ 이 언어({lang_info['name']})는 발음 재생을 지원하지 않습니다.",
                    fg=C["warn"]
    ...
    def play_tts(self):
        ...
        gtts_code = self.current_lang_info["gtts"]
        cache_path = CACHE_DIR / f"{gtts_code}_{hashlib.md5(text.encode('utf-8')).hexdigest()}.mp3"
        ...
        # _play_audio 메서드는 별도의 스레드에서 TTS(gTTS 또는 edge-tts)를 사용하여 현재 문장을 음성으로 재생합니다.
        def _play_audio():
            try:
                _synthesize_and_play(text, gtts_code, cache_path)
            except (ValueError, TTSUnavailableError):
                # gTTS/edge-tts 둘 다 해당 언어를 지원하지 않는 경우
